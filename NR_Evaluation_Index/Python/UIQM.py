...
# 获取UIQM 越大越好
def get_UIQM(res_dir):
    uiqms = []
    # 导入测试集
    images = glob.glob(res_dir + "*")
    # 遍历每个图像
    for img in images:
        x = Image.open(img).resize((350, 350)).convert('RGB')
        # x = Image.open(img).convert('RGB')
        x = to_tensor(x).cuda().unsqueeze(0)
        x = torch.cat((x, x), 0)
        res = torch.stack([torch_uiqm(res) for res in x], dim=0)
        uiqms.append(res[0].item())

    return np.mean(np.array(uiqms))


def get_One_UIQM(imgPath):
    # x = Image.open(imgPath).resize((256, 256)).convert('RGB')
